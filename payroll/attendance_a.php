        ...
        if ($queryleave->num_rows >= 1){
            ...
            if ($equery->num_rows > 0){
                ...
                if( $erow['time_out']== $timeoutvalue){
                        $sqlup = "UPDATE attendance SET time_out = '$logout' WHERE time_out = '$timeoutvalue' AND date = '$date_now' AND employee_id = '$eids'";
                        if ($conn->query($sqlup)) {
                ...
                $sql = "INSERT INTO attendance (employee_id, date, time_in,time_out, status, num_hr,num_ot) 
                    VALUES ('$eids', '$date_now', '$login', '$logout','$logstatus','$set','$set')";
